...
module.exports = async function firstAccess(req, res) {
  ...
  const verification = await fetch(`${url}/auth/v1/verify`, {
    ...
    body: JSON.stringify({ email, token, type: requestedType })
  });
  const verificationPayload = await verification.json().catch(() => ({}));
  const accessToken = verificationPayload.access_token || "";
  if (!verification.ok || !accessToken) {
    const errorCode = verificationPayload.error_code || verificationPayload.code || "invalid_or_expired_code";
