...
async function sheetsRequest(pathname, options = {}) {
  ...
  if (!response.ok) throw new Error(`Sheets falhou: ${response.status} ${await response.text()}`);
  return response.json();
}

async function main() {
  const spreadsheet = await sheetsRequest("?fields=sheets(properties(sheetId,title))");
