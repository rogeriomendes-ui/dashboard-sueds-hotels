...
function buildPortalAccess(profile) {
  ...
  const hotelModule = (profile?.hotels || []).map((hotel) => ({
    PLAZA: "opinarios_plaza",
    CABRALIA: "opinarios_cabralia",
    SEGUNDO_SOL: "opinarios_segundo_sol",
    PREMIUM: "opinarios_premium",
    TRANCOSO: "opinarios_trancoso",
    CASAS: "opinarios_casas"
  })[hotel.code]).find(Boolean);
  const landingPage = environments.painel_gestores ? "/gestores"
    : environments.inspecoes ? "/gestores?modulo=inspecoes"
      : environments.ranking_vendedores ? "/gestores?modulo=ranking_vendedores"
        : environments.opinarios_rede ? "/gestores?modulo=opinarios_rede"
          : environments.opinarios_hotel && hotelModule ? `/gestores?modulo=${hotelModule}`
            : environments.redes_sociais ? "/gestores?modulo=redes_sociais"
              : environments.marketing_competitividade ? "/gestores?modulo=marketing_competitividade"
                : environments.tv_vendedores ? "/gestores?modulo=tv_vendedores"
                  : environments.treinamentos ? "/Treinamentos"
